...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ## the idea is to use (x-y)^2=x^2+y^2
        sq_X_train=np.sum(self.X_train**2,axis=1).reshape(1,num_train)
        sq_X=np.sum(X**2,axis=1).reshape(num_test,1)
        X_X_train= X@ self.X_train.T
        dists=np.sqrt(sq_X_train+sq_X-2*X_X_train)
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i,j]=np.dot(X[i],self.X_train[j])/(np.sqrt((np.sum(X[i]**2)))*np.sqrt((np.sum(self.X_train[j]**2))))
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            dists[i]=self.X_train @ (X[i].T)/(np.sqrt(np.sum(self.X_train**2,axis=1)) * np.sqrt(np.sum(X[i]**2)))
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        norms=(np.sqrt(np.sum(X**2,axis=1)).reshape(X.shape[0],1) @ np.sqrt(np.sum(self.X_train**2,axis=1)).reshape(self.X_train.shape[0],1).T)
        dists=X @ self.X_train.T  /norms
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            closest_y=self.y_train[ranking][:k]
            ...
            highest_freq=np.max(counts)
            
            y_pred[i]=np.min(unique[counts==highest_freq])
